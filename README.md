...
本轮实现聚焦在明天中午前可提交、可运行、可验证的最小版本：

- 在移动端 Demo 底部导航新增“安排”入口。
- 新增安排列表页，支持查看标题、时间、地点、相关人、来源和当前状态。
- 支持手动创建安排，数据暂存在浏览器 localStorage。
- 支持进入安排详情页，查看基础信息和相关对话上下文。
- 支持“完成安排”和“以后再说”两个轻量处理动作。
- 过期但未完成的安排只做弱化展示，不做强红色警告，避免制造焦虑。
- 在 `/sendtest` 中加入本地规则模拟 AI 识别：
  - 输入“后天去一趟医院”会生成“去医院”安排。
  - 输入“爸爸：一定记得去医院”时，如果已有未完成的“去医院”安排，会把该消息合并进同一条安排的相关对话上下文。
  - 安排详情页可查看这些来源对话。

## MVP 取舍

当前版本先不接入真实大模型 API、不做日历视图、不做提醒循环、不做自动完成判断，也不处理复杂群聊归属、多人互相安排、AI 代执行等高级场景。安排的数据结构中先保留时间、地点、相关人、来源和上下文，为后续扩展留出空间。

## AI 识别模拟方式

本轮使用本地字符串规则模拟 AI：`/sendtest` 发送消息后会同步调用本地识别函数，命中指定文本时写入 `arkme-demo.arrangements` localStorage，并通过浏览器事件通知移动端 Demo 刷新。这样可以先验证“消息识别 -> 生成/合并安排 -> 详情可追溯上下文”的核心闭环。

## 后续迭代计划

后续建议按小版本继续推进：

1. 接入用户自填 API Key 的真实大模型识别能力。
2. 补充更多私聊、群聊、发给自己等识别场景，并建立合并置信度与人工确认机制。
3. 增加日历视图、提醒设置、时间段安排和无明确时间安排的整理方式。
4. 基于对话上下文探索自动完成、AI 可协助执行、AI 可全自动执行的分层处理。
5. 打磨“以后再说”的后续流转，例如沉淀区、轻提醒或重新浮现策略。
